from dataset.ibm_dataset import IBMDataset
...
class IBMSpeechStream(object):
    ...
    def __init__(self, configuration, gpu_ids, use_cuda):
        ibm = IBM(configuration['data_root'], ratio=configuration['train_val_split'])
        ...
        self._speaker_dic = ibm.speaker_dic
    ...
    def export_to_features(self, ibm_path, configuration):
        if not os.path.isdir(ibm_path):
            raise ValueError("IBM dataset not found at path '{}'".format(ibm_path))

        # Create the features path directory if it doesn't exist
        features_path = ibm_path + os.sep + configuration['features_path']
