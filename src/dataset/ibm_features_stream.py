...
class IBMFeaturesStream(object):

    def __init__(self, ibm_path, configuration, gpu_ids, use_cuda):
        self._normalizer = None
        if configuration['normalize']:
            with open(configuration['normalizer_path'], 'rb') as file:
                self._normalizer = pickle.load(file)

        self._training_data = IBMFeaturesDataset(ibm_path, 'train', self._normalizer, features_path=configuration['features_path'])
        self._validation_data = IBMFeaturesDataset(ibm_path, 'val', self._normalizer, features_path=configuration['features_path'])
        ...
        self._speaker_dic = self._make_speaker_dic(ibm_path + os.sep + 'raw' + os.sep + 'IBM-Corpus')
        self.ibm_path = ibm_path
